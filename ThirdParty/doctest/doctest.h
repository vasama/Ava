...
namespace detail {
    ...
    class DOCTEST_INTERFACE ValidationMessageBuilder
    {
    public:
        ValidationMessageBuilder(const char* file, int line, assertType::Enum severity);
        ValidationMessageBuilder() = delete;
        ~ValidationMessageBuilder();

        DOCTEST_DELETE_COPIES(ValidationMessageBuilder);

        template<typename T>
        ValidationMessageBuilder& operator<<(const T& in) {
            this->m_message_builder << in;
            return *this;
        }
...
class DOCTEST_INTERFACE TestCaseValidation
{
public:
    TestCaseValidation(const TestCaseData& data);
    TestCaseValidation() = delete;

    DOCTEST_DELETE_COPIES(TestCaseValidation);

    detail::ValidationMessageBuilder addMessage() const;
    detail::ValidationMessageBuilder addFailure() const;

private:
    const TestCaseData* m_test_case_data;
};

struct DOCTEST_INTERFACE IValidator
{
    virtual void test_case_start(const TestCaseData&) = 0;
    virtual void test_case_end(const TestCaseValidation&) = 0;
...
namespace detail {
    ...
    struct ContextState : ContextOptions, TestRunStats, CurrentTestCaseStats
    {
        ...
        std::vector<IValidator*> validators_currently_used;
...
void Context::addValidator(IValidator&) {}
...
namespace detail {
#define DOCTEST_ITERATE_THROUGH_VALIDATORS(function, ...)                                          \
    for(auto& curr_val : g_cs->validators_currently_used)                                          \
    curr_val->function(__VA_ARGS__)
...
namespace detail {
    ...
    ValidationMessageBuilder::ValidationMessageBuilder(const char* file, int line, assertType::Enum severity)
        : m_message_builder(file, line, severity) {}
    
    ValidationMessageBuilder::~ValidationMessageBuilder() {
        m_message_builder.log();
    }
} // namespace detail

TestCaseValidation::TestCaseValidation(const TestCaseData& data)
    : m_test_case_data(&data) {}

detail::ValidationMessageBuilder TestCaseValidation::addMessage() const {
    return detail::ValidationMessageBuilder(m_test_case_data->m_file,
        m_test_case_data->m_line, assertType::Enum::is_warn);
}
detail::ValidationMessageBuilder TestCaseValidation::addFailure() const {
    return detail::ValidationMessageBuilder(m_test_case_data->m_file,
        m_test_case_data->m_line, assertType::Enum::is_require);
}
namespace {
    std::mutex g_mutex;
...
void Context::addValidator(IValidator& validator) {
    p->validators_currently_used.push_back(&validator);
}
    ...
    for(auto& curr : testArray) {
        ...
        // execute the test if it passes all the filtering
        {
            ...
            do {
                ...
                DOCTEST_ITERATE_THROUGH_VALIDATORS(test_case_start, tc);
                ...
                {
                    TestCaseValidation validation(tc);
                    DOCTEST_ITERATE_THROUGH_VALIDATORS(test_case_end, validation);
                }
